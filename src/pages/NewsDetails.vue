    ...
    <Row class="public-row">
      ...
      <Col :xs="0" :sm="23" :md="19" :lg="15" class="carousel-col-height-aoto">
        <div class="case-details-info">
          ...
          <div class="case-author"> 作者：<span>{{newsObj.author}}</span> | <span>{{newsObj.createTime}}</span> | <span>
        ...
        <div class="case-details-recommend">
          <Tabs>
            <TabPane label="热门推荐">
              <div class="recommend-title" v-for="item in newsRecommendList" @click="jumpRecommend(item.aid)">{{item.title}}</div>
            </TabPane>
            <TabPane label="最新资讯">
              <div class="recommend-title" v-for="item in newestNewsList" @click="jumpRecommend(item.aid)">{{item.title}}</div>
            </TabPane>
          </Tabs>
        </div>
      </Col>
  ...
  export default {
    ...
    , methods:{
      ...
      newsDetails: function (caseAid) {
        ...
        this.$network.post(that.$GLOBAL.newsDetails, newsParam, null, function (data) {
          that.newsObj = data;
      ...
      , newestNews: function () {
        ...
        this.$network.post(that.$GLOBAL.newestNews, newestNewsParam, null, function (data) {
